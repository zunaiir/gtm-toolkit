...
def generate_brief(company_name, website_url, contact_name, contact_title):
    ...
    prompt = f"""You are helping an Account Executive at Tiger Data prepare for a discovery call.

Tiger Data is a high-performance PostgreSQL platform built for transactional, analytical, and AI/agentic
workloads. It helps engineering teams handle real-time analytics, time-series data, and vector search at
scale. Think: the database layer that powers modern, data-intensive applications.
...
### Likely Tech Stack
[Based on available signals, what tech are they probably running? Be specific if you can infer it.]

### Data & Infrastructure Pain Points
[What database/data scale challenges might a company like this face? Root cause, not surface symptoms.]

### Tiger Data Fit
[How could Tiger Data specifically solve their problems — be concrete, not generic]
...
def generate_cold_emails(company_name, website_url, contact_name, contact_title, custom_notes):
    """Generate 3 cold email variations for a Tiger Data prospect."""
    website_content = fetch_website(website_url)
    news_content    = fetch_news(company_name)
    ...
    prompt = f"""You are helping an Account Executive at Tiger Data write cold outbound emails.

Tiger Data is a PostgreSQL platform that helps engineering teams reduce latency, eliminate data system sprawl,
and consolidate fragmented infrastructure into one fast, reliable database.
...
- Each email follows one simple structure: one observation, one specific pain (latency, data drift, system sprawl, or similar), one outcome, one ask
...
- Sign off with: [Your name] at Tiger Data
...
def generate_crm_summary(call_notes, company_name, contact_name, contact_title, call_date):
    ...
    prompt = f"""You are helping an Account Executive at Tiger Data log a sales call in their CRM.

Tiger Data is a high-performance PostgreSQL platform for transactional, analytical, and AI/agentic workloads.
...
### Pain Points
[Bullet list of specific pains the prospect mentioned]
...
**Metrics:** [What quantifiable outcomes did they mention? Revenue, latency, cost, time saved?]
**Economic Buyer:** [Who controls the budget? Identified or unknown?]
**Decision Criteria:** [What will they use to evaluate solutions?]
**Decision Process:** [How will they decide? Timeline? Who else is involved?]
**Identify Pain:** [What is the core business pain driving this evaluation?]
